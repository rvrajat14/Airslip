...
Each operation lives under the `IdentityApi` namespace and within an optional tag: `IdentityApi(.tagName).operationId`. If an operation doesn't have an operationId one will be generated from the path and method.
...
let getUserRequest = IdentityApi.User.GetUser.Request(id: 123)
...
Each [Request](#request) also has a `makeRequest` convenience function that uses `IdentityApi.shared`.
...
There are some options to control how invalid JSON is handled when decoding and these are available as static properties on `IdentityApi`:
...
- **IdentityApi.Identity**
	- **GetV1IdentityGoogleLogin**: GET `/v1/identity/google-login`
	- **GetV1IdentityLogout**: GET `/v1/identity/logout`
	- **PostV1IdentityGoogle**: POST `/v1/identity/google`
	- **PostV1IdentityLogin**: POST `/v1/identity/login`
	- **PostV1IdentityRefresh**: POST `/v1/identity/refresh`
	- **PutV1IdentityBiometric**: PUT `/v1/identity/biometric`
- **IdentityApi.UserProfile**
	- **GetV1Profile**: GET `/v1/profile`
	- **GetV1ProfilePhoto**: GET `/v1/profile/photo`
	- **PostV1ProfilePhoto**: POST `/v1/profile/photo`
	- **PutV1Profile**: PUT `/v1/profile`
